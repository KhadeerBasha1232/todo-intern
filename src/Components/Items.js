...
import { Pagination, Chip, Button, ButtonGroup, Slide } from '@mui/material';
...
const DataTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`https://todo-kb-intern.onrender.com/api/main/todos/?page=${currentPage}&todosPerPage=${rowsPerPage}`, {
          headers: headers,
        });
  ...
  const renderPriorityCell = (params) => {
    ...
    let priorityText = '';
    let priorityColor = ''; // Assign an empty string initially
  
    switch (priority) {
      case 0:
        priorityText = 'Low';
        priorityColor = 'default'; // Set to a valid color for 'Low' priority
        break;
      case 1:
        priorityText = 'Mid';
      ...
      default:
        ...
        priorityColor = 'default'; // Handle default case explicitly
        break;
    }
    ...
    const handleDeleteClick = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Fetch updated data
          const updatedResponse = await fetch(`https://todo-kb-intern.onrender.com/api/main/todos/?page=${currentPage}&todosPerPage=${rowsPerPage}`, {
            headers: headers,
          });

          const updatedData = await updatedResponse.json();

          setRows(updatedData.todos);
  ...
  return (
    <div style={{ width: '90%', height: 'auto', margin: '0 auto', textAlign: 'center' }}>
      <Slide
        ...
        onExited={handleExit}
      >
        <div style={{height: rows.length === 0 ? '300px' : 'auto',}}>
          <DataGrid
            rows={rows}
            columns={columns}
            {...rows}
