...
const HighPriorityTodos = () => {
  const [lowPriorityRows, setlowPriorityRows] = useState([]);
  ...
  const fetchHighPriorityTodos = async () => {
    try {
      ...
      const response = await fetch(`https://todo-kb-intern.onrender.com/api/main/lowtodo`, {
        headers: headers,
      });

      const data = await response.json();

      setlowPriorityRows(data.lowPriorityTodos);
    } catch (error) {
    ...
    const handleDeleteClick = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Fetch updated data
          const updatedResponse = await fetch(`https://todo-kb-intern.onrender.com/api/main/lowtodo`, {
            headers: headers,
          });

          const updatedData = await updatedResponse.json();

          setlowPriorityRows(updatedData.lowPriorityTodos);
        } else {
  ...
  const renderFooter = () => {
    return (
      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px' }}>
        <div>Total Rows : {lowPriorityRows.length}</div>
  ...
  return (
    <div style={{ width: '90%', height: 'auto', marginBottom:"120px !important", margin: '0 auto', textAlign: 'center' }}>
      <Slide
        direction="left"
        in={true}
        mountOnEnter
        ...
        onExited={handleExit}
      >
        <div style={{height: lowPriorityRows.length === 0 ? '300px' : 'auto',marginBottom:"100px"}}>
          <DataGrid
            ...
            slots={{
              ...
              noRowsOverlay: () => (
                <div style={{ textAlign: 'center', padding: '20px' }}>
                  <p>No low Priority todos available. Add a new one!</p>
                  <Button
                    variant="contained"
                    color="primary"
