...
const HighPriorityTodos = () => {
  const [highPriorityRows, setHighPriorityRows] = useState([]);
  ...
  const fetchHighPriorityTodos = async () => {
    try {
      ...
      const response = await fetch(`https://todo-kb-intern.onrender.com/api/main/hightodo`, {
        headers: headers,
      });

      const data = await response.json();

      setHighPriorityRows(data.highPriorityTodos);
    } catch (error) {
    ...
    const handleDeleteClick = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Fetch updated data
          const updatedResponse = await fetch(`https://todo-kb-intern.onrender.com/api/main/hightodo`, {
            headers: headers,
          });

          const updatedData = await updatedResponse.json();

          setHighPriorityRows(updatedData.highPriorityTodos);
        } else {
  ...
  const renderFooter = () => {
    return (
      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px' }}>
        <div>Total Rows : {highPriorityRows.length}</div>
  ...
  return (
    <div style={{ width: '90%', height: '100%', margin: '0 auto', textAlign: 'center' }}>
      <Slide
        ...
        onExited={handleExit}
      >
        <div style={{height: highPriorityRows.length === 0 ? '300px' : 'auto',}}>
          <DataGrid
            ...
            slots={{
              ...
              noRowsOverlay: () => (
                <div style={{ textAlign: 'center', padding: '20px' }}>
                  <p>No High Priority todos available. Add a new one!</p>
                  <Button
                    variant="contained"
                    color="primary"
